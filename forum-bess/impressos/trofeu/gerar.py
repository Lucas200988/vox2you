...
def placa(hom, linha2, nome_arq):
    ...
    s.append(f'<text x="{W/2}" y="{H-24}" font-size="4.2" font-weight="800" letter-spacing="1" fill="{GREY}">28 · 09 · 2026 · CUIABÁ — MT</text>')
    s.append('</g>')
    ...
    s.append(f'<rect x="{W/2-20}" y="86" width="40" height="18" rx="2" fill="none" stroke="{GREY}" stroke-width=".4" stroke-dasharray="1.5 1"/>')
    s.append(f'<text x="{W/2}" y="97" font-size="3" fill="{GREY}" text-anchor="middle">LOGO {hom} · peça colada · 40 × 18 mm</text>')
    s.append('</g>')
    # ---- COTAS (nao imprimir)
