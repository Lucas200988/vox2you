...
/** Distinct from the dev seed's `mock-phone`: channel external ids are globally unique. */
export const TEST_PHONE_NUMBER_ID = 'mock-phone-integration'
...
export async function setupTestEnv(): Promise<TestEnv> {
  ...
  const seed = await seedVox2you(db, {
    ...
    unitSlug: 'test-unit',
    channelExternalId: TEST_PHONE_NUMBER_ID,
  })
  ...
  return {
    ...
    deps,
    realtime,
    processor: new InboundProcessor(deps, realtime),
    seed,
...
export async function inbound(
  ...
  from: string,
  text: string,
  opts: { name?: string; referral?: Record<string, string> } = {},
) {
