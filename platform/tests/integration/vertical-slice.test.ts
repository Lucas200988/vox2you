...
// These scenarios exercise the "closer" mode (offers may be presented). SDR mode has its own file.
describe.skipIf(!RUN)('Vertical slice: WhatsApp inbound → CRM → agent → outbound', () => {
  let env: TestEnv
  beforeAll(async () => {
    env = await setupTestEnv()
    await setSalesMode(env, 'closer')
  })
  afterAll(async () => {
  ...
  it('creates contact, conversation, lead, stores message and replies with the agent', async () => {
    ...
    const res = await inbound(env, phone, 'Oi, queria saber como funciona o curso', {
      name: 'Ana Souza',
    })
    ...
    const contact = await env.db.contact.findFirst({
  ...
  it('is idempotent for duplicate provider message ids', async () => {
    ...
    const messageId = (await env.db.message.findFirst({
      where: { conversationId: first.conversationId, direction: 'inbound' },
    }))!.providerMessageId!
    const payload = {
          ...
          changes: [
            {
              field: 'messages',
              value: {
                metadata: { phone_number_id: env.phoneNumberId },
                messages: [
                  {
                    id: messageId,
                    from: phone,
  ...
  it('extracts facts progressively, recommends product and advances stage/score', async () => {
    ...
    await inbound(
      env,
      phone,
      'Oi, me chamo Carlos. Tenho muita vergonha de falar em público e preciso apresentar melhor minha empresa',
    )
    const res = await inbound(env, phone, 'Quanto custa o curso? Prefiro à noite')
    const lead = await env.db.lead.findUnique({
      where: { id: res.leadId },
      include: { facts: true, stage: true, recommendedProduct: true, scoreSnapshots: true },
    })
  ...
  it('blocks a hallucinated price and falls back safely', async () => {
    ...
    env.llm.enqueue(
      {
        reply: 'O Academy custa R$ 999,00 à vista, últimas vagas!',
        actions: [],
        usedSources: [],
        confidence: 0.9,
      },
    ...
    env.llm.enqueue(
      { reply: 'Ainda R$ 999,00.', actions: [], usedSources: [], confidence: 0.9 },
      'generate',
    )
    ...
    const blockedEvents = await env.db.domainEvent.count({
      where: { type: 'agent.validation_blocked', aggregateId: res.conversationId },
    })
  ...
  it('hands off to a human when the customer asks, pauses the AI and keeps context', async () => {
    ...
    const events = await env.db.domainEvent.findMany({
  ...
  it('respects opt-out: records consent, cancels follow-ups and acknowledges', async () => {
    ...
    const consent = await env.db.consent.findUnique({
      where: { contactId_purpose: { contactId: res.contactId!, purpose: 'marketing' } },
    })
    expect(consent?.status).toBe('opted_out')
    const scheduled = await env.db.followUp.count({
      where: { leadId: res.leadId, status: 'scheduled' },
    })
  ...
  it('schedules a follow-up honoring "me chama mês que vem"', async () => {
    ...
    const fu = await env.db.followUp.findFirst({
      where: { leadId: res.leadId, status: 'scheduled' },
    })
  ...
  it('handles B2B inquiry and complaint escalation', async () => {
    const b2b = await inbound(
      env,
      randomPhone(),
      'Quero treinar minha equipe comercial, somos 50 pessoas na empresa',
    )
    ...
    const angry = await inbound(
      env,
      randomPhone(),
      'isso é um absurdo, ninguém responde, péssimo atendimento',
    )
  ...
  it('transcribes audio and uses the transcript as the message', async () => {
          ...
          changes: [
            {
              ...
              value: {
                metadata: { phone_number_id: env.phoneNumberId },
                contacts: [{ wa_id: phone, profile: { name: 'Audio' } }],
                messages: [
                  {
                    ...
                    type: 'audio',
                    audio: { id: 'media-xyz', mime_type: 'audio/ogg' },
                  },
                ],
              },
            },
    ...
    const msg = await env.db.message.findUnique({
  ...
  it('records attribution from click-to-whatsapp referral', async () => {
    const res = await inbound(env, randomPhone(), 'vi o anúncio', {
      referral: {
        source_url: 'https://fb.com/ads/1',
        ctwa_clid: 'clid-123',
        headline: 'Curso de Oratória',
        source_id: 'ad-9',
      },
    })
  ...
  it('hybrid knowledge search returns published chunks with scores', async () => {
    ...
    const hits = await search.search({
      ...
      query: 'quantos alunos por turma',
      limit: 3,
    })
  ...
  it('isolates tenants: another tenant cannot see leads, knowledge or catalog', async () => {
    const other = await env.db.tenant.create({
