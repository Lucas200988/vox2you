...
export interface FollowUpPlanInput {
  ...
  policy: { maxAttempts: number; strategies: Record<string, { delayHours?: number; goal?: string }> }
  now?: Date
}
...
const DEFAULT_STRATEGIES: Record<string, { delayHours: number; goal: string }> = {
  price_then_silence: { delayHours: 22, goal: 'Retomar ligando o investimento ao objetivo e propor visita' },
  abandoned_scheduling: { delayHours: 4, goal: 'Facilitar a escolha do horário oferecendo 2 opções' },
  asked_later: { delayHours: 72, goal: 'Retomar com leveza e conteúdo de valor' },
  qualified_waiting_payment: { delayHours: 48, goal: 'Confirmar se conseguiu resolver e oferecer ajuda' },
...
export function planFollowUp(input: FollowUpPlanInput): FollowUpPlan {
  ...
  if (['won', 'lost'].includes(input.stageKey)) return { schedule: false, reason: `stage_${input.stageKey}` }
  if (input.hasScheduledAppointment) return { schedule: false, reason: 'appointment_scheduled' }
  if (input.attemptsSoFar >= input.policy.maxAttempts) return { schedule: false, reason: 'max_attempts_reached', strategy: 'task_for_human' }
  ...
  if (dnc && dnc > now) {
    return { schedule: true, scheduledAt: dnc, reason: 'customer_asked_later', goal: 'Retomar conforme combinado', scenario: 'asked_later', strategy: 'template' }
  }
  if (input.agentRequested) {
    return { schedule: true, scheduledAt: addHours(now, input.agentRequested.hours), reason: input.agentRequested.reason, goal: strategies['generic']?.goal, scenario: 'agent_requested', strategy: 'text' }
  }
  ...
  else if (intent === 'booking_request' || input.stageKey === 'scheduling') scenario = 'abandoned_scheduling'
  else if (signals.includes('later') || intent === 'objection') scenario = signals.includes('later') ? 'asked_later' : 'objection_open'
  else if (input.stageKey === 'negotiation') scenario = 'qualified_waiting_payment'
  else if (intent === 'opt_out' || intent === 'human_request' || intent === 'complaint') return { schedule: false, reason: `intent_${intent}` }
  else if (intent === 'off_topic' || intent === 'smalltalk') return { schedule: false, reason: 'no_commercial_context' }
  ...
  return { schedule: true, scheduledAt: addHours(now, delay), reason: `scenario:${scenario}`, goal: strategy.goal, scenario, strategy: 'text' }
}
