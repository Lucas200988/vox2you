...
export class FollowUpExecutor {
  ...
  async execute(
    ...
    const fu = await db.followUp.findUnique({
      where: { id: followUpId },
      include: {
        lead: { include: { contact: true, unit: true, stage: true } },
    ...
    if (fu.lead.doNotContactUntil && fu.lead.doNotContactUntil > new Date())
      return skip('do_not_contact_until')
    const conversation =
      fu.conversation ??
      (await db.conversation.findFirst({
        where: { leadId: fu.leadId, status: 'open' },
    ...
    if (conversation.lastInboundAt && conversation.lastInboundAt > fu.createdAt)
      return skip('customer_replied')
    ...
    if (!windowOpen) {
      ...
      if (!template) {
        await createTask(db, {
          data: {
            tenantId: fu.tenantId,
            leadId: fu.leadId,
            title: 'Follow-up fora da janela de 24h sem template aprovado: contatar manualmente',
            kind: 'message',
            priority: 'normal',
            createdBy: 'agent',
            assigneeId: fu.lead.ownerId,
          },
        })
        return skip('window_closed_no_template')
      }
      const product = fu.lead.interestProductId
        ? await db.product.findUnique({ where: { id: fu.lead.interestProductId } })
        : null
      const vars = [
        fu.lead.contact.firstName ?? 'tudo bem',
        product?.name ?? 'seu desenvolvimento em comunicação',
      ]
      const msg = await outbound.send(
        ...
        conversation.id,
        {
          ...
          templateVariables: vars.slice(0, template.variables.length || 2),
        },
        'agent',
      )
    ...
    const user = PromptRegistry.render(prompt.content, {
      scenario: fu.scenario ?? 'generic',
      goal: fu.goal ?? 'retomar a conversa',
      facts: renderFacts(memory.facts),
      summary: memory.conversation.summary ?? '(sem resumo)',
      catalog_summary: AgentTools.renderCatalog(catalog, {
        hidePricing: settings.salesMode === 'sdr',
      }),
      last_messages: renderHistory(memory.recent.slice(-6), 1500),
    ...
    const guard = runGuardrails({
      ...
      maxChars: 400,
      maxQuestions: 1,
      timezone: fu.lead.unit.timezone,
    })
    if (!guard.ok) {
      ...
      await createTask(db, {
        data: {
          tenantId: fu.tenantId,
          leadId: fu.leadId,
          title: `Follow-up bloqueado pela validação (${guard.issues.map((i) => i.code).join(',')}): enviar manualmente`,
          kind: 'message',
          priority: 'normal',
          createdBy: 'agent',
          assigneeId: fu.lead.ownerId,
    ...
    const msg = await outbound.send(
      ctx,
      conversation.id,
      { text: guard.rewrittenReply ?? res.data.reply },
      'agent',
    )
    await db.followUp.update({
      where: { id: followUpId },
      data: { draftMessage: guard.rewrittenReply ?? res.data.reply },
    })
    await service.markSent(followUpId, msg.id)
    await db.agentRun.create({
      data: {
        tenantId: fu.tenantId,
        unitId: fu.lead.unitId,
        ...
        decision: 'reply',
        decisionReason: `followup:${fu.scenario ?? 'generic'}`,
      },
    })
    return { status: 'sent', messageId: msg.id }
  }
