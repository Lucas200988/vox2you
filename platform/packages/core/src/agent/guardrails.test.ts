    ...
    offers: [
      {
        ...
        validTo: null,
        display: 'R$ 3.990,00 ou 12x de R$ 332,50',
      },
    ],
    classes: [
      {
...
describe('runGuardrails (SDR mode)', () => {
  it('blocks any price, installment or discount even when it exists in the catalog', () => {
    const r = runGuardrails({
      ...base,
      salesMode: 'sdr',
      reply: 'O Academy custa R$ 3.990,00 ou 12x de R$ 332,50, com 10% de desconto à vista.',
    })
    ...
    expect(codes).toEqual(
      expect.arrayContaining([
        'sdr_price_mention',
        'sdr_installment_mention',
        'sdr_discount_mention',
      ]),
    )
  })
  it('lets a visit invitation through and keeps time validation against slots', () => {
    const slots = [
      {
        start: new Date('2026-09-15T21:00:00.000Z'),
        end: new Date('2026-09-15T22:00:00.000Z'),
        calendarId: 'cal',
      },
    ]
    const r = runGuardrails({
      ...
      slots,
      reply:
        'Faz sentido querer saber. Os valores a gente apresenta na visita, depois de um diagnóstico rápido. Tenho terça às 17:00 livre para você conhecer a unidade. Posso reservar?',
    })
    expect(r.ok).toBe(true)
    expect(r.issues.filter((i) => i.severity === 'block')).toEqual([])
  })
  it('does not apply the SDR rules in closer mode', () => {
    const r = runGuardrails({
      ...base,
      salesMode: 'closer',
...
describe('runGuardrails', () => {
  it('accepts prices and times that exist in the catalog', () => {
    const r = runGuardrails({
      ...base,
      reply: 'O Academy custa R$ 3.990,00 ou 12x de R$ 332,50, com turma às 19:00.',
    })
    expect(r.ok).toBe(true)
    expect(r.issues.filter((i) => i.severity === 'block')).toEqual([])
  })
  it('blocks unsupported prices, times, discounts and guarantees', () => {
    const r = runGuardrails({
      ...base,
      reply:
        'Custa R$ 2.500,00 com 30% de desconto, turma às 20:30, resultado garantido, últimas vagas!',
    })
    const codes = r.issues.map((i) => i.code)
    expect(r.ok).toBe(false)
    expect(codes).toEqual(
