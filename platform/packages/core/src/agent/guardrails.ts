...
export interface GuardrailInput {
  ...
  maxQuestions: number
  timezone: string
  /** In SDR mode any price, installment or discount mention is blocked, catalog or not. */
  salesMode?: 'sdr' | 'closer'
}

const SDR_COMMERCIAL_TERMS =
  /(parcel|mensalidade|à vista|a vista|desconto|promo[cç]|investimento de|custa)/i
const INSTALLMENT_RE = /\b\d{1,2}\s?x\b(?!\s?(mais|vezes mais|por semana|na semana))/i

const FORBIDDEN_PATTERNS: Array<{ re: RegExp; code: string; message: string }> = [
  {
    re: /(garantimos|garantido|garantia de resultado|resultado garantido|100% de sucesso)/i,
    code: 'guaranteed_result',
    message: 'Promessa de resultado garantido',
  },
  {
    re: /(últimas? vagas?|ultimas? vagas?|só hoje|somente hoje|última chance|ultima chance|oferta acaba)/i,
    code: 'false_scarcity',
    message: 'Escassez sem fonte (só permitido se vier do catálogo)',
  },
  {
    re: /\b(ignore|desconsidere) (as|todas as) instruções/i,
    code: 'injection_echo',
    message: 'Eco de instrução injetada',
  },
]
...
export function runGuardrails(input: GuardrailInput): ValidationResult {
  ...
  // 0. SDR mode: the agent books the visit and never talks money, whatever the sources say
  if (input.salesMode === 'sdr') {
    const money = extractMoneyMentions(reply)
    if (money.length)
      issues.push({
        code: 'sdr_price_mention',
        severity: 'block',
        message: `Modo SDR: valores ("${money[0]}") só são apresentados na visita`,
      })
    if (INSTALLMENT_RE.test(reply))
      issues.push({
        code: 'sdr_installment_mention',
        severity: 'block',
        message: 'Modo SDR: não cite parcelamento',
      })
    if (/(\d{1,2})\s?%/.test(reply) && /desconto|off|abatimento/i.test(reply))
      issues.push({
        code: 'sdr_discount_mention',
        severity: 'block',
        message: 'Modo SDR: não cite desconto',
      })
    if (SDR_COMMERCIAL_TERMS.test(reply))
      issues.push({
        code: 'sdr_commercial_terms',
        severity: 'warn',
        message: 'Modo SDR: evite discutir condições comerciais; conduza para a visita',
      })
  }
  ...
  const knowledgeAmounts = new Set(
    extractMoneyMentions(knowledgeText)
      .map(parseBrlAmount)
  ...
  for (const mention of extractMoneyMentions(reply)) {
    ...
    if (!inCatalog && !inKnowledge) {
      issues.push({
        ...
        severity: 'block',
        message: `Valor "${mention}" não existe no catálogo vigente`,
      })
    } else if (!inCatalog && inKnowledge) {
      issues.push({
        code: 'price_from_knowledge',
        severity: 'warn',
        message: `Valor "${mention}" veio de documento, não do catálogo (catálogo prevalece)`,
      })
    }
  }
  ...
  if (pct.length) {
    const maxAllowed = Math.max(
      0,
      ...(input.catalog ?? []).flatMap((p) => p.offers.map((o) => o.maxDiscountPct)),
    )
    if (/desconto|off|abatimento/i.test(reply) && pct.some((p) => p > maxAllowed))
      issues.push({
        code: 'unauthorized_discount',
        severity: 'block',
        message: `Desconto de ${Math.max(...pct)}% acima do permitido (${maxAllowed}%)`,
      })
  }
  ...
  for (const p of input.catalog ?? [])
    for (const c of p.classes) allowedTimes.add(c.startTime).add(c.endTime)
  ...
  for (const t of extractTimeMentions(reply)) {
    if (!allowedTimes.has(t))
      issues.push({
        code: 'unsupported_time',
        severity: 'block',
        message: `Horário ${t} não consta em turmas, agenda ou documentos`,
      })
  }
  ...
  if (reply.length > input.maxChars) {
    issues.push({
      code: 'too_long',
      severity: 'warn',
      message: `Resposta com ${reply.length} caracteres (máx ${input.maxChars})`,
    })
  }

  // 6. Question count
  const q = countQuestions(reply)
  if (q > input.maxQuestions)
    issues.push({
      code: 'too_many_questions',
      severity: q > input.maxQuestions + 1 ? 'warn' : 'info',
      message: `${q} perguntas na mesma mensagem`,
    })
  ...
  const questions = reply
    ...
    .filter((s) => s.includes('?'))
    .map((s) => normalize(s))
  const previousAgentQuestions = input.memory.recent
  ...
  for (const qn of questions) {
    if (previousAgentQuestions.some((p) => similarity(p, qn) > 0.8))
      issues.push({
        code: 'repeated_question',
        severity: 'warn',
        message: 'Pergunta já feita anteriormente',
      })
    if (asksKnownFact(qn, input.memory))
      issues.push({
        code: 'asks_known_fact',
        severity: 'warn',
        message: 'Pergunta cuja resposta já está nos fatos do lead',
      })
  }

  // 8. Robotic opener
  if (/^(olá|oi)[!,.]?\s+(como posso (te )?ajud)/i.test(reply))
    issues.push({ code: 'robotic_opener', severity: 'info', message: 'Abertura genérica' })
...
const FACT_QUESTION_HINTS: Array<{ key: string; re: RegExp }> = [
  ...
  {
    key: 'profile_type',
    re: /(pessoal ou (para|pra) (sua )?empresa|para voc[eê] ou (para|pra) (o|seu) time)/,
  },
