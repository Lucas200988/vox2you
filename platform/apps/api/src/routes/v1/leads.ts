...
export const leadRoutes: FastifyPluginAsync = async (app) => {
  ...
  const publish = (leadId: string, unitId: string, payload: Record<string, unknown>) => app.ctx.realtime.publish({ type: 'lead.updated', tenantId: '', unitId, leadId, payload, at: new Date().toISOString() })

  app.get('/leads', { schema: { tags: ['crm'], querystring: z.object({ unitId: z.string().uuid().optional(), stageKey: z.string().optional(), stageId: z.string().uuid().optional(), ownerId: z.string().uuid().optional(), status: z.string().optional(), minScore: z.coerce.number().optional(), q: z.string().optional(), productId: z.string().uuid().optional(), tagId: z.string().uuid().optional(), limit: z.coerce.number().int().min(1).max(500).optional() }) }, preHandler: [app.requireAuth(), app.requireScope('leads:read')] }, async (req) => {
    return { items: await leads.list(req.auth!, req.query as Parameters<LeadService['list']>[1]) }
  })

  app.get('/leads/kanban', { schema: { tags: ['crm'], querystring: z.object({ unitId: z.string().uuid() }) }, preHandler: app.requireAuth('leads:read') }, async (req) => {
    return leads.kanban(req.auth!, (req.query as { unitId: string }).unitId)
  })

  app.get('/leads/:id', { schema: { tags: ['crm'], params: IdParams }, preHandler: [app.requireAuth(), app.requireScope('leads:read')] }, async (req) => {
    return leads.get(req.auth!, (req.params as z.infer<typeof IdParams>).id)
  })

  app.patch('/leads/:id', { schema: { tags: ['crm'], params: IdParams, body: LeadUpdateSchema }, preHandler: [app.requireAuth('leads:write'), app.requireScope('leads:write')] }, async (req) => {
    ...
    const updated = await leads.update(req.auth!, id, {
      ...(body.ownerId !== undefined ? { owner: body.ownerId ? { connect: { id: body.ownerId } } : { disconnect: true } } : {}),
      ...(body.interestProductId !== undefined ? { interestProduct: body.interestProductId ? { connect: { id: body.interestProductId } } : { disconnect: true } } : {}),
      ...
      ...(body.nextFollowupReason !== undefined ? { nextFollowupReason: body.nextFollowupReason } : {}),
    })
  ...
  app.post('/leads/:id/facts', { schema: { tags: ['crm'], params: IdParams, body: FactInputSchema }, preHandler: app.requireAuth('leads:write') }, async (req) => {
    return leads.addFact(req.auth!, (req.params as z.infer<typeof IdParams>).id, req.body as z.infer<typeof FactInputSchema>)
  })

  app.delete('/leads/:id/facts/:factId', { schema: { tags: ['crm'], params: IdParams.extend({ factId: z.string().uuid() }) }, preHandler: app.requireAuth('leads:write') }, async (req) => {
    const p = req.params as { id: string; factId: string }
    return leads.rejectFact(req.auth!, p.id, p.factId)
  })

  app.post('/leads/:id/notes', { schema: { tags: ['crm'], params: IdParams, body: z.object({ body: z.string().min(1) }) }, preHandler: app.requireAuth('leads:write') }, async (req) => {
    return leads.addNote(req.auth!, (req.params as z.infer<typeof IdParams>).id, (req.body as { body: string }).body)
  })

  app.post('/leads/:id/tags', { schema: { tags: ['crm'], params: IdParams, body: z.object({ name: z.string().min(1) }) }, preHandler: app.requireAuth('leads:write') }, async (req) => {
    return leads.addTag(req.auth!, (req.params as z.infer<typeof IdParams>).id, (req.body as { name: string }).name)
  })

  app.delete('/leads/:id/tags/:tagId', { schema: { tags: ['crm'], params: IdParams.extend({ tagId: z.string().uuid() }) }, preHandler: app.requireAuth('leads:write') }, async (req) => {
  ...
  app.get('/leads/:id/followups', { schema: { tags: ['crm'], params: IdParams }, preHandler: app.requireAuth('leads:read') }, async (req) => {
    return { items: await new FollowUpService(app.ctx.db).listForLead(req.auth!, (req.params as z.infer<typeof IdParams>).id) }
  })

  app.post('/leads/:id/followups/:followUpId/cancel', { schema: { tags: ['crm'], params: IdParams.extend({ followUpId: z.string().uuid() }) }, preHandler: app.requireAuth('leads:write') }, async (req) => {
    const p = req.params as { id: string; followUpId: string }
    const fu = await app.ctx.db.followUp.findFirst({ where: { id: p.followUpId, leadId: p.id, tenantId: req.auth!.tenantId } })
    if (!fu) throw new NotFoundError('FollowUp', p.followUpId)
    return app.ctx.db.followUp.update({ where: { id: fu.id }, data: { status: 'cancelled', skipReason: `user:${req.auth!.userId}` } })
  })

  app.get('/pipelines', { schema: { tags: ['crm'] }, preHandler: app.requireAuth('leads:read') }, async (req) => {
  ...
  // Tasks
  app.get('/tasks', { schema: { tags: ['crm'], querystring: z.object({ assigneeId: z.string().uuid().optional(), status: z.enum(['open', 'done', 'cancelled']).optional(), leadId: z.string().uuid().optional() }) }, preHandler: app.requireAuth('leads:read') }, async (req) => {
    const q = req.query as { assigneeId?: string; status?: string; leadId?: string }
    return { items: await app.ctx.db.task.findMany({ where: { tenantId: req.auth!.tenantId, status: q.status ?? 'open', ...(q.assigneeId ? { assigneeId: q.assigneeId } : {}), ...(q.leadId ? { leadId: q.leadId } : {}) }, include: { lead: { select: { id: true, contact: { select: { name: true, phone: true } } } }, assignee: { select: { id: true, name: true } } }, orderBy: [{ priority: 'desc' }, { dueAt: 'asc' }], take: 200 }) }
  })
  ...
  app.post('/tasks/:id/complete', { schema: { tags: ['crm'], params: IdParams }, preHandler: app.requireAuth('leads:write') }, async (req) => {
    ...
    return app.ctx.db.task.update({ where: { id }, data: { status: 'done', completedAt: new Date() } })
  })
}
