...
export function startSchedulers(ctx: WorkerContext): () => void {
  ...
  every(ctx.config.OUTBOX_POLL_MS, 'dispatch-outbox', async () => {
    ...
    await ctx.queues.events.addBulk(
      events.map((e) => ({
        name: e.type,
        data: { eventId: e.id },
        opts: {
  ...
  every(ctx.config.FOLLOWUP_POLL_MS, 'due-followups', async () => {
    ...
    await ctx.queues.followups.addBulk(
      due.map((f) => ({
        name: 'followup',
        data: { followUpId: f.id },
        opts: {
          jobId: `fu-${f.id}`,
          attempts: 3,
  ...
  // Visit reminders (24h / 2h before) and outcomes (prompt after 2h, no-show after 24h)
  every(10 * 60 * 1000, 'visit-reminders', async () => {
    const reminders = new VisitReminderService(ctx.db, ctx.providers, ctx.logger, ctx.realtime)
    const sent = await reminders.sendDueReminders()
    if (sent.length) ctx.logger.info({ sent }, 'visit reminders sent')
    const outcomes = await reminders.handleOverdue()
  ...
  every(10 * 60 * 1000, 'sla-check', async () => {
    const stale = await ctx.db.lead.findMany({
      where: { status: 'open', stage: { maxHoursInStage: { not: null } } },
      include: { stage: true },
      take: 500,
    })
    const now = Date.now()
    for (const lead of stale) {
      ...
      const recent = await ctx.db.domainEvent.findFirst({
        where: {
          type: 'lead.inactive',
          aggregateId: lead.id,
          occurredAt: { gte: new Date(now - 24 * 36e5) },
        },
      })
      if (recent) continue
      await ctx.db.domainEvent.create({
        data: {
          ...
          payload: {
            ...
            hoursInStage: Math.round((now - lead.stageEnteredAt.getTime()) / 36e5),
            limit,
